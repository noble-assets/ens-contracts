...
export default deployScript(
  async ({ deploy, get, read, execute: write, tx, namedAccounts, network }) => {
    ...
    if (resolver === zeroAddress) {
      console.warn(
        `  - WARN: No resolver set for .noble; not setting interface ${interfaceId} for NameWrapper`,
      )
      return
    }

    // Set interface on the resolver configured for .noble
    const ownedResolver =
      get<(typeof artifacts.OwnedResolver)['abi']>('OwnedResolver')
    console.log(
      `  - Setting NameWrapper interface ID ${interfaceId} on .noble resolver`,
    )
    await tx({
      ...
      data: encodeFunctionData({
        ...
        args: [namehash('noble'), interfaceId, nameWrapper.address],
      }),
      account: owner,
    })
